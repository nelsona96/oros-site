 ...
 * only renders this component while the modal should be open — there's no
 * local open/closed state to track. `open` stays `true` for the component's
 * whole lifetime; closing (Escape, backdrop click, or the close button, all
 * funneled through `onOpenChange`) calls `router.back()`, which is Next's
 * own documented pattern for a modal built on intercepting + parallel
 * routes — popping the URL back to `/portfolio/videos` is what actually
  ...
  return (
    ...
    >
      <DialogContent
        ...
        className="rounded-lg rounded-control border-border bg-app-bg w-full max-w-4xl gap-4 p-4 ring-0 sm:max-w-4xl sm:p-6"
      >
